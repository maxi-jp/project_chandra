...
namespace IS_XNA_Shooter
{
    ...
    class ParticleSystemCamera
    {
        ...
        public int MAX_ACELERATION_X = 3;
        public int MAX_ACELERATION_Y = 2;
                ...
                if (timeSinceLastParticle >= PARTICLE_CREATION_INTERVAL)
                {
                    ...
                    if (particles[i].actualAge >= particles[i].deadAge)
                    {
                        ...
                        particles[i].aceleration = new Vector2
                            (
                                -(float)Math.Cos(sourceRotation) * ObtainRandom(MAX_ACELERATION_X),
                                -(float)Math.Sin(sourceRotation) * ObtainRandom(MAX_ACELERATION_Y)
                            );
